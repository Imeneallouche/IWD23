...
from models import StripeModel, OrderModel
...
class CreateCardTokenView(APIView):
    ...
    def post(self, request):
        ...
        if len(customer_data) == 0:
            ...
            # holds card number (last four digits)
        ...
        if card_invalid:
            ...
            if cardStatus:
                try:
                    save_card_in_db(data, email, create_user_card.id,
                                    customer["id"], request.user)
                    message = {
                        "customer_id": customer["id"], "email": email, "card_data": create_user_card}
...
class DeleteCardView(APIView):
    ...
    def post(self, request):
        data = request.data
        obj_card = StripeModel.objects.get(
